...
\begin{itemize}
    ...
    \item (Exercise 4.98)The statement of \textbf{Theorem 4.9} is incomplete, but the proof is given to you. Fill in the blank in the statement of the theorem, and provide a few sentences supporting your answer.
    \item (Exercise 4.99) Discuss how \textbf{Theorem 4.9} greatly simplifies the PageRank iterative process described previously. In other words, there is no reason to iterate at all. Instead, find ... what?
    ...
    \begin{itemize}
        ...
        \item Find steady state PageRank vector using the two methods described: one using the iterative difference equation and the other using \textbf{Theorem 4.9} and the dominant eigenvector.
...
\begin{enumerate}
    ...
    \item The difference equation $\boldsymbol{X}_{n+1}=\boldsymbol{H}\boldsymbol{x}_n$ is used to iteratively refine the estimates of the page ranks. You can view the iterations as a person visiting a page and then following a link at random, then following a random link on the next page, and the next, and the next, etc. Hence we see that the iterations evolve exactly as expected for a difference equation.
            \begin{table}[H]
                ...
                \hline
                0 & $\boldsymbol{x}_0$ \\
                1 & $\boldsymbol{x}_1 = \boldsymbol{H} \boldsymbol{x}_0$ \\
                2 & $\boldsymbol{x}_2 = \boldsymbol{H} \boldsymbol{x}_1 = \boldsymbol{H}^2 \boldsymbol{x}_0$ \\
                3 & $\boldsymbol{x}_3 = \boldsymbol{H} \boldsymbol{x}_2 = \boldsymbol{H}^3 \boldsymbol{x}_0$ \\
                4 & $\boldsymbol{x}_4 = \boldsymbol{H} \boldsymbol{x}_3 = \boldsymbol{H}^4 \boldsymbol{x}_0$ \\
                $\vdots$ & $\vdots$ \\
...
\noindent First note that $\boldsymbol{A}$ is an $n\times n$ stochastic matrix so from \textbf{Theorem 4.8}. We know that there are $n$ linearly independent eigenvectors.
We can then substitute the eigenvalues from \textbf{Theorem 4.8} in \textbf{Theorem 4.7}.
